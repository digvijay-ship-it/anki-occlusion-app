...
class GDriveService:
    ...
    def _get_or_create_subfolder(self, headers, folder_name, parent_id):
        ...
        try:
            ...
            r = session.post(create_url, headers=headers, json=metadata, timeout=15)
            if r.status_code == 200:
    ...
    def _upload_resumable_stream(self, headers, local_file_path, folder_id, drive_filename, file_id=None):
        ...
        try:
            ...
            # Read file bytes into memory to release file handle immediately, avoiding Windows file locking during network I/O
            if file_size <= 50 * 1024 * 1024:
                ...
                while uploaded < file_size:
                    chunk = file_bytes[uploaded:uploaded + CHUNK_SIZE]
                    chunk_len = len(chunk)
                    if chunk_len == 0:
                        break
