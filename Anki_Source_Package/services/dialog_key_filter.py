...
class UniversalDialogKeyFilter(QObject):
    ...
    and automatically flushes process memory (fitz.TOOLS.store_shrink & gc.collect)
    whenever any QDialog or QMessageBox closes.
    """
        ...
        if event.type() == QEvent.Close:
            if isinstance(obj, (QDialog, QMessageBox)):
                try:
                    ...
                    flush_process_memory(f"{obj.__class__.__name__} Closed")
                except Exception:
                    pass
