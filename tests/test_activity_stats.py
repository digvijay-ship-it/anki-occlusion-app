...
class TestActivityStats(unittest.TestCase):
    ...
    def test_grouped_card_counted_as_single_review(self):
        # A card with 8 grouped masks (e.g. group_id="table_group") reviewed together
        grouped_data = {
            "decks": [
                {
                    "name": "Playing Cards",
                    "cards": [
                        {
                            "_id": "card_52",
                            "boxes": [
                                {"group_id": "table_group", "reviewed_at": "2026-08-27 10:00:00", "last_quality": 4, "box_id": f"b_{i}"}
                                for i in range(8)
                            ]
                        },
                        {
                            "_id": "card_ungrouped",
                            "boxes": [
                                {"group_id": "", "reviewed_at": "2026-08-27 10:05:00", "last_quality": 5, "box_id": "single_1"},
                                {"group_id": "", "reviewed_at": "2026-08-27 10:06:00", "last_quality": 5, "box_id": "single_2"}
                            ]
                        }
                    ],
                    "children": []
        ...
        stats = get_daily_activity_stats("2026-08-27", grouped_data)
        
        # 1 from 8-box grouped card + 2 from ungrouped boxes = 3 total reviews (NOT 10!)
        self.assertEqual(stats["total"], 3)
        self.assertEqual(stats["good"], 1)   # from the 1 grouped card
        self.assertEqual(stats["easy"], 2)   # from the 2 ungrouped cards

        deck_node = stats["tree"][0]
        self.assertEqual(deck_node["total_reviews"], 3)
