...
class DojoAssets:
    ...
    def __init__(self):
        ...
        self._ui_icon_cache: dict[tuple, "QPixmap"] = {}
        self._clan_icon_cache: dict[tuple, "QPixmap"] = {}
    ...
    def get_ui_icon(self, index, size=64):
        ...
        painter.end()
        self._ui_icon_cache[key] = out_pix
        return out_pix

    def get_clan_icon(self, name, size=48):
        if self.clan_sheet.isNull():
            return QPixmap()
        key = (name.lower(), size)
        if key in self._clan_icon_cache:
            return self._clan_icon_cache[key]
        w = 1376 // 2
        h = 768 // 2
        ...
        painter.end()
        self._clan_icon_cache[key] = out_pix
        return out_pix
    ...
    def get_turtle_widget(self):
        ...
        w.setStyleSheet(
            "background:rgba(20, 20, 31, 0.6); border: 1px solid #A86CFF; border-radius: 8px; margin-top: 6px; margin-bottom: 6px;"
        )
        ql = QHBoxLayout(w)
        ...
        lbl.setStyleSheet(
            "color:#A86CFF; font-size:10px; font-weight:bold; border:none; background:transparent;"
        )
        ql.addWidget(icon)
        ql.addWidget(lbl)
        return w
    ...
    def get_pizza_widget(self):
        ...
        w.setStyleSheet(
            "background:rgba(20, 20, 31, 0.6); border: 1px solid #72FF4F; border-radius: 8px; margin-top: 6px; margin-bottom: 6px;"
        )
        pl = QHBoxLayout(w)
        ...
        lbl.setStyleSheet(
            "color:#72FF4F; font-size:10px; font-weight:bold; border:none; background:transparent;"
        )
