...
class ScratchpadOverlay(QWidget):
    ...
    def mousePressEvent(self, e):
        ...
        if e.button() == Qt.LeftButton:
            if self.mode == "eraser":
                ...
                path_ms = (time.perf_counter() - t_calc0) * 1000.0
                try:
                    from services.pen_profiler import pen_profiler
    ...
    def mouseMoveEvent(self, e):
        if self.mode == "eraser" and (e.buttons() & Qt.LeftButton):
            ...
            if dx * dx + dy * dy < 2.0:
                try:
                    from services.pen_profiler import pen_profiler
                    pen_profiler.record_move(pos.x(), pos.y(), pressure=1.0, accepted=False)
                except Exception:
                    pass
                e.accept()
                return
            ...
            try:
                from services.pen_profiler import pen_profiler
                pen_profiler.record_move(pos.x(), pos.y(), pressure=1.0, accepted=True, path_calc_ms=path_ms)
            except Exception:
                pass
    ...
    def mouseReleaseEvent(self, e):
        if e.button() == Qt.LeftButton:
            if self.mode != "eraser" and self.current_stroke:
                ...
                try:
                    from services.pen_profiler import pen_profiler
                    last_pt = pts[-1] if pts else QPoint(0, 0)
    ...
    def tabletEvent(self, e):
        ...
        pos = e.posF().toPoint()
        pressure = float(e.pressure()) if hasattr(e, "pressure") else 1.0
        if e.type() == QEvent.TabletPress:
            ...
            if self.mode == "eraser" and (e.buttons() & Qt.LeftButton or e.pressure() > 0):
                ...
                if dx * dx + dy * dy < 2.0:
                    try:
                        from services.pen_profiler import pen_profiler
                        pen_profiler.record_move(pos.x(), pos.y(), pressure=pressure, accepted=False)
                    except Exception:
                        pass
                    e.accept()
                    return
                ...
                try:
                    from services.pen_profiler import pen_profiler
                    pen_profiler.record_move(pos.x(), pos.y(), pressure=pressure, accepted=True, path_calc_ms=path_ms)
                except Exception:
                    pass
